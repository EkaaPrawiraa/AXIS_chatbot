...
class TestSubjectWriter:
    async def test_subject_upsert_updates_sentiment_and_mentions(
        self, neo4j_client: nc.Neo4jClient, test_namespace: dict
    ) -> None:
        ...
        row = await neo4j_client.execute_read_single(
            """
            MATCH (u:User {id: $uid})-[r:HAS_SUBJECT]->(p:Subject {id: $pid})
...
class TestRelationshipBuilders:
    ...
    async def test_contextual_links_and_recurring_theme(
        ...
        row = await neo4j_client.execute_read_single(
            """
            MATCH (e:Experience {id: $exp})-[:INVOLVES_SUBJECT]->(p:Subject {id: $per})
