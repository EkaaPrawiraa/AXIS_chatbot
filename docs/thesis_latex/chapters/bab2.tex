...
Nilai tambah komponen rekayasa diuji melalui ablasi yang mengubah satu faktor dan membekukan faktor lainnya. Pada perbandingan retrieval vektor saja dan hibrid, korpus, kueri, model, prompt, parameter pembangkitan, jumlah kandidat, dan batas konteks dibuat sama. Setiap kueri menjadi pasangan observasi, lalu selisih metrik dilaporkan bersama interval kepercayaan 95\% berbasis \textit{bootstrap}. Untuk keluaran yang memerlukan penilaian ordinal, konfigurasi \textit{LLM-as-judge} menggunakan pedoman anotasi tertulis yang sama dan menyimpan keluaran per kasus. Jika lebih dari satu penilai digunakan, kesepakatannya dapat dilaporkan dengan \textit{weighted kappa} untuk dua penilai atau Krippendorff's alpha ketika jumlah penilai dan ketersediaan label lebih kompleks.

Karena model eksternal dapat berubah, keterulangan bertumpu pada pencatatan konfigurasi, bukan pada asumsi layanan selalu identik. Setiap eksperimen perlu mencatat penyedia, nama atau snapshot model, tanggal eksekusi, versi 	extit{prompt}, parameter pembangkitan, model 	extit{embedding}, snapshot korpus, dan keluaran per kasus. Bukti otomatis dapat mendukung klaim validitas rekayasa dan kualitas relatif komponen, tetapi tidak dapat menggantikan bukti pengalaman pengguna, efektivitas klinis, atau dampak longitudinal.
...
Kajian pada bab ini memperlihatkan bahwa setiap kemampuan chatbot pendamping membawa manfaat sekaligus batas. Respons empatik dapat terasa suportif tetapi tetap generik. Skrining percakapan dapat menurunkan hambatan, tetapi perlu mempertahankan batas yang jelas dari diagnosis. Memori dapat meningkatkan kontinuitas, sekaligus menambah persoalan relevansi dan privasi. Graf memperjelas hubungan, tetapi membutuhkan ekstraksi dan pemeliharaan, sedangkan vektor lebih fleksibel terhadap teks yang belum terstruktur. Interaksi suara dapat menurunkan hambatan bercerita, tetapi menambah pengolahan data dan waktu pemrosesan.

Temuan dan pertukaran tersebut menjadi dasar analisis kebutuhan serta keputusan rancangan pada BAB III. Posisi kontribusi rekayasa penelitian telah dinyatakan pada penutup BAB I agar bab ini tetap berfungsi sebagai kajian pustaka.
