...
Redis digunakan sebagai lapisan pendukung, bukan kontribusi utama penelitian. Perannya adalah menyimpan \textit{state} sementara dan membantu pengendalian penggunaan agar layanan tidak mudah disalahgunakan. Pembatasan teknis ini mendukung penggunaan purwarupa tanpa mengambil fokus dari percakapan, mood dan PHQ-9, serta memori jangka panjang hibrid.

Purwarupa AXIS dijalankan pada domain publik (\url{https://axis-chatbot.my.id}) dengan sertifikat HTTPS. \textit{Reverse proxy} menangani penerbitan dan pembaruan sertifikat serta memisahkan lalu lintas ke layanan \textit{backend} dan antarmuka web. Ketersediaan ini menyiapkan akses bagi evaluasi pengguna nyata pada penelitian lanjutan tanpa memerlukan konfigurasi jaringan khusus dari partisipan.
...
Saat afek akut muncul, sistem memprioritaskan \textit{grounding}. Distorsi kognitif yang terdeteksi pada giliran tersebut tetap dicatat oleh penilai dialog.

Pada giliran berikutnya, sistem menawarkan \textit{reframing} apabila pengguna masih membawa muatan emosional yang sama dan belum berpindah topik. Mekanisme susulan ini diverifikasi melalui pemanggilan LLM nyata dan dilaporkan pada Subbab~\ref{sec:evaluasi-fungsional}.

Sistem membatasi penyebutan nama pengguna dengan memeriksa dua giliran asisten sebelumnya. Jika nama baru saja digunakan, instruksi kontekstual mencegah pengulangannya pada respons berikutnya.

Pemeriksaan berbasis kode juga digunakan untuk mengurangi pola respons yang berulang. Sistem mendeteksi rangkaian giliran yang terus ditutup dengan pertanyaan dan pembuka yang menggaungkan bentuk serupa. Instruksi berikutnya kemudian meminta variasi penutup atau pembuka. Efektivitas mekanisme tersebut diukur pada Subbab~\ref{sec:evaluasi-fungsional}.

Prinsip yang sama diterapkan pada rujukan memori. Jika pengalaman atau pola yang sama baru saja disebut, sistem meminta pembangkit respons tidak mengulang rujukan tersebut pada giliran berikutnya.
...
Rancangan memahami lalu menyuarakan pada Subbab~\ref{sec:rancangan-companionship} direalisasikan sebagai konfigurasi \textit{pipeline} alternatif. Pada konfigurasi dasar, pembangkit respons menerima konteks memori dan menyusun balasan dalam satu langkah. Pada konfigurasi alternatif, satu node pemahaman internal dijalankan lebih dahulu dengan profil model yang lebih ringan.

Keluaran node pemahaman tidak ditampilkan kepada pengguna. Pembangkit respons tetap bertanggung jawab atas nada, batas topik, dan larangan istilah klinis. Kedua konfigurasi dibandingkan pada Subbab~\ref{sec:v2-dialogue}.
...
Antarmuka AXIS disediakan sebagai aplikasi web dengan pengalaman mobile sebagai prioritas. Bagian ini menjelaskan tindakan yang tersedia bagi pengguna, bukan rincian pembangunan komponen \textit{frontend}. Chat, mood dan PHQ-9, serta memori menjadi antarmuka bagi tiga kemampuan inti. Interaksi suara, profil, bantuan, hotline, dan pengaturan menjadi fitur pendukung untuk akses, keselamatan, transparansi, dan kendali pengguna.
...
Sebagai fitur pendukung, \textit{Confession Space} menyediakan ruang suara sementara. Pengguna berbicara melalui mikrofon, melihat transkripsi, lalu menerima balasan dalam bentuk suara dan teks. Isi percakapan tidak ditulis ke riwayat permanen atau memori jangka panjang AXIS.

Batas tersebut berlaku pada penyimpanan utama AXIS, bukan sebagai jaminan anonimitas menyeluruh. Audio atau teks tetap dapat diproses oleh penyedia STT dan TTS sesuai konfigurasi layanan, sedangkan \textit{guardrail} tetap aktif ketika muncul sinyal risiko. Tampilan \textit{Confession Space} ditunjukkan pada Gambar \ref{fig:impl-voice}.
