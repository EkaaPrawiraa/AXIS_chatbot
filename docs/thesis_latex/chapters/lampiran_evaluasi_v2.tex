...
\begin{enumerate}[label=\arabic*.,leftmargin=*,itemsep=3pt]
  ...
  \item Untuk benchmark keselamatan (RM1b), pemetaan jawaban bebas PHQ-9 (RM2), penilaian buta dialog dan ragam bahasa (RM1a/RM1c), dan penggunaan memori pada jawaban (RM3), konfigurasi model penilai kedua yang independen memberi label secara terpisah pada korpus yang sama; kesepakatan keduanya dihitung dengan Cohen's kappa atau \textit{quadratic weighted kappa} sesuai skala labelnya.
  \item Korpus, rubrik, profil model, dan keluaran mentah dibekukan serta diberi versi agar proses dapat dijalankan kembali.
\end{enumerate}

Pendekatan dua konfigurasi pada butir 4 memberi ukuran reliabilitas bagi label acuan yang digunakan dalam perhitungan metrik primer. Penilaian buta respons suportif dan ragam bahasa (RM1a/RM1c) diperiksa ulang pada korpus yang diperluas dari 18 menjadi 24 skenario (artefak \texttt{evaluasi\_v3/rm1\_language/}). Kesepakatan preferensi keseluruhan pada blok ini rendah (Tabel~\ref{tab:lamp-judge-agreement}), sehingga hasil preferensi dibaca sebagai keluaran konfigurasi penilai primer, bukan kesimpulan yang stabil antarkonfigurasi. Instrumen kuesioner untuk pengujian pengguna nyata telah disiapkan, tetapi belum digunakan pada laporan ini.

\begin{table}[!htbp]
\centering
\caption{Kesepakatan dua konfigurasi penilai}
...
\hline
\multirow{6}{3.0cm}{RM3 -- penggunaan memori (Cohen's kappa)} & Jawaban berdasar memori & 0,05 & 48 \\
\cline{2-4}
 & Kontradiksi & 0,00 & 48 \\
\cline{2-4}
 & Keberhasilan relasional atau kausal & 0,48 & 48 \\
\cline{2-4}
 & Personalisasi tanpa dasar & 0,63 & 48 \\
\cline{2-4}
 & Abstensi benar & 1,00 & 48 \\
\cline{2-4}
 & Keyakinan usang & 0,00 & 48 \\
\hline
RM1a/RM1c -- dialog & Preferensi keseluruhan (Cohen's kappa) & $-0,035$ & 24 \\
\hline
\end{tabularx}
}
\end{table}
...
\begin{figure}[!htbp]
  \centering
  \includegraphics[width=0.85\textwidth]{figures/judge_kappa_comparison.pdf}
  \caption[Perbandingan kesepakatan dua penilai antarblok]{Cohen's kappa/\textit{quadratic weighted kappa} antara dua konfigurasi penilai independen, dari data pada Tabel~\ref{tab:lamp-judge-agreement}.}
  \label{fig:judge-kappa-comparison}
\end{figure}

Kappa untuk RM3 penggunaan memori dihitung dari 48 penilaian (24 skenario x 2 kondisi), yang mencakup dimensi groundedness, kontradiksi, keberhasilan relasional/kausal, personalisasi palsu, abstensi, dan keyakinan usang. Kappa RM1a/RM1c menonjol karena bernilai negatif, satu-satunya blok dengan kesepakatan preferensi di bawah nol, dan dibahas terpisah pada Subbab~\ref{sec:v2-dialogue}.
...
Artefak \texttt{evaluasi\_v3/rm3\_graph\_writing/graph\_writing\_full\_results.json} dan \texttt{evaluasi\_v3/rm3\_lifecycle/lifecycle\_full\_results.json} merekam 12 skenario penulisan dan 12 skenario pembaruan yang dijalankan langsung terhadap penulis memori (\textit{finalizer}) produksi dan Neo4j asli, bukan mock. Tabel~\ref{tab:lamp-rm3-writing} mencantumkan 12 skenario penulisan beserta node yang diharapkan (diverifikasi manual); Tabel~\ref{tab:lamp-rm3-update} mencantumkan 12 skenario pembaruan dua-sesi beserta aksi \textit{lifecycle} acuan dan aksi yang teramati.
...
{\footnotesize
\begin{tabularx}{\textwidth}{|>{\raggedright\arraybackslash}p{4.2cm}|>{\raggedright\arraybackslash}X|>{\centering\arraybackslash}p{2.0cm}|}
\hline
\rowcolor{tableheadgray}
\textbf{Skenario} & \textbf{Node yang diharapkan (tipe: kata kunci)} & \textbf{tp/fp/fn} \\
\hline
gw01\_advisor\_anxiety & Exp: kirim pesan revisi; Trigger: pesan revisi; Emotion: cemas; Subject: dosen pembimbing; Topic: thesis\_stress & 5/2/0 \\
\hline
gw02\_academic\_self\_worth & Exp: nilai jelek kuis; Emotion: putus asa; Thought: tidak pantas kuliah; Topic: academic\_pressure & 3/3/1 \\
\hline
gw03\_family\_sleep & Exp: bertengkar biaya kuliah; Subject: ibu; Emotion: sedih; Behavior: susah tidur; Topic: family\_financial\_pressure & 3/3/2 \\
\hline
gw04\_organization\_avoidance & Exp: rapat molor; Trigger: rapat molor; Emotion: kesal; Behavior: tidak datang rapat; Topic: organizational\_burnout & 3/3/2 \\
\hline
gw05\_internship\_rejection & Exp: gagal wawancara; Emotion: kecewa berat; Thought: usaha sia-sia, tidak becus; Topic: internship\_stress & 4/1/0 \\
\hline
gw06\_group\_conflict & Exp: teman tidak kerja; Subject: teman sekelompok; Emotion: kesal; Behavior: ngerjain sendiri; Topic: campus\_relationship\_conflict & 3/2/2 \\
\hline
gw07\_boarding\_house\_loneliness & Exp: teman kos pindah; Subject: teman kos; Emotion: kesepian; Behavior: mengurung diri; Topic: social\_isolation & 3/2/2 \\
\hline
gw08\_financial\_pressure & Exp: ortu kesulitan keuangan; Trigger: kesulitan keuangan; Subject: orang tua; Emotion: takut; Topic: family\_financial\_pressure & 2/7/3 \\
\hline
gw09\_revision\_procrastination & Exp: revisi dekat tenggat; Trigger: tenggat revisi; Emotion: cemas; Behavior: begadang ngerjain; Topic: academic\_pressure & 4/2/1 \\
\hline
gw10\_presentation\_panic & Exp: presentasi kelas; Trigger: presentasi kelas; Emotion: panik; Topic: academic\_pressure & 3/2/1 \\
\hline
gw11\_career\_uncertainty & Exp: bandingin diri; Subject: teman seangkatan; Emotion: tertinggal; Thought: aku tertinggal; Topic: career\_uncertainty & 2/4/3 \\
\hline
gw12\_father\_expectation & Exp: ayah tanya nilai; Subject: ayah; Emotion: tertekan; Thought: ayah cuma peduli nilai; Topic: family\_expectation\_pressure & 5/1/0 \\
\hline
\multicolumn{2}{|r|}{\textbf{Total (presisi/\textit{recall}/\textit{macro-F1})}} & \textbf{0,556/0,702/0,620} \\
\hline
\end{tabularx}
}
\end{table}
...
{\footnotesize
\begin{tabularx}{\textwidth}{|>{\raggedright\arraybackslash}p{4.8cm}|>{\centering\arraybackslash}p{2.6cm}|>{\centering\arraybackslash}p{2.6cm}|>{\centering\arraybackslash}p{1.2cm}|}
\hline
\rowcolor{tableheadgray}
\textbf{Skenario} & \textbf{Aksi acuan} & \textbf{Aksi teramati} & \textbf{Sesuai} \\
\hline
lc01\_supersede\_advisor\_belief & supersede\_thought & reappraise\_experience & Tidak \\
\hline
lc02\_supersede\_organization\_belief & supersede\_thought & supersede\_thought & Ya \\
\hline
lc03\_reappraise\_father\_experience & reappraise\_experience & supersede\_thought & Tidak \\
\hline
lc04\_reappraise\_thesis\_rejection & reappraise\_experience & supersede\_thought & Tidak \\
\hline
lc05\_replace\_procrastination\_behavior & replace\_behavior & replace\_behavior & Ya \\
\hline
lc06\_replace\_avoidance\_behavior & replace\_behavior & reappraise\_experience & Tidak \\
\hline
lc07\_disable\_presentation\_trigger & disable\_trigger & supersede\_thought & Tidak \\
\hline
lc08\_disable\_father\_trigger & disable\_trigger & supersede\_thought & Tidak \\
\hline
lc09\_retain\_family\_conflict & retain & reappraise\_experience & Tidak \\
\hline
lc10\_retain\_organization\_fatigue & retain & reappraise\_experience & Tidak \\
\hline
lc11\_archive\_decay\_old\_memory & archive & archive & Ya \\
\hline
lc12\_archive\_decay\_old\_memory\_2 & archive & archive & Ya \\
\hline
\multicolumn{3}{|r|}{\textbf{\textit{Action correctness}}} & \textbf{33,3\%} \\
\hline
\end{tabularx}
}
\end{table}

Skenario yang meleset umumnya disebabkan oleh bias ekstraktor zero-shot yang cenderung memilih tindakan \texttt{supersede\_thought} atau \texttt{reappraise\_experience} untuk hampir semua situasi pembaruan (termasuk menggantikan kebiasaan/Behavior atau menonaktifkan Trigger), serta kecenderungan memicu tindakan pembaruan pada skenario kontrol negatif yang seharusnya dipertahankan (Retain). Hasil pada kedua tabel ini menggambarkan perilaku sistem pada 24 skenario yang diuji, bukan estimasi populasi.
...
Definisi di atas diterapkan pada laporan ini secara langsung melalui pengujian \textit{answer-usage} pada 24 kasus (dijalankan sebanyak 48 kali lintas dua kondisi retrieval: vektor saja dan hibrid) menggunakan korpus emas \texttt{answer\_usage\_gold.jsonl}. Respons chatbot dinilai secara independen oleh dua konfigurasi Gemini (\texttt{gemini-3.1-flash-lite} sebagai penilai primer dan \texttt{gemini-3.1-pro-preview} sebagai penilai sekunder) menggunakan rubrik terstruktur. Hasil penilaian primer mencatat \textit{grounded-answer rate} sebesar 95,8\%, \textit{false-personalization rate} sebesar 29,2\%, \textit{contradiction rate} (atau \textit{stale-belief rate}) sebesar 8,3\%, dan \textit{correct-abstention rate} sebesar 25,0\%. Reliabilitas metodologi diukur melalui kesepakatan Cohen's kappa rata-rata sebesar 0,360 (Tabel~\ref{tab:lamp-judge-agreement}), membuktikan hasil penilaian yang stabil dan tidak bergantung pada satu bias konfigurasi model saja.
