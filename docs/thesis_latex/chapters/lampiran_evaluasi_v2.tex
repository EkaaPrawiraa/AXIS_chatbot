...
Lampiran ini memuat definisi korpus, pedoman anotasi, rubrik, dan protokol eksperimen. Seluruh tabel hasil pada Bab IV diisi setelah evaluasi dijalankan.
...
Setiap tugas anotasi menggunakan pedoman tertulis yang dapat dijalankan oleh minimal dua konfigurasi model penilai (\textit{LLM-as-judge}) independen \parencite{liu2023geval,zheng2023judging}. Prosedurnya adalah sebagai berikut.

\begin{enumerate}[label=\arabic*.,leftmargin=*,itemsep=3pt]
  \item Setiap konfigurasi model penilai menerima definisi label dan contoh positif serta negatif melalui \textit{system prompt} yang sama.
  \item Setiap konfigurasi model penilai mengerjakan sampel kalibrasi terhadap label acuan yang sudah diketahui; sampel ini tidak dimasukkan ke hasil akhir.
  \item Setiap konfigurasi model penilai memberi label secara independen pada korpus evaluasi.
  \item Kesepakatan antarkonfigurasi dihitung sebelum penentuan label akhir.
  \item Perbedaan label antarkonfigurasi diselesaikan oleh model penilai ketiga sebagai penentu.
  \item Korpus yang telah disepakati dibekukan dan diberi versi.
\end{enumerate}

\begin{table}[!htbp]
\centering
\caption{Pemilihan ukuran kesepakatan antarkonfigurasi model penilai}
{\footnotesize
\begin{tabularx}{\textwidth}{|>{\raggedright\arraybackslash}p{3.2cm}|>{\raggedright\arraybackslash}X|}
\hline
\rowcolor{tableheadgray}
\textbf{Kondisi} & \textbf{Ukuran yang digunakan} \\
\hline
Dua konfigurasi model penilai, label ordinal & \textit{Weighted kappa} \\
\hline
Dua konfigurasi model penilai, label nominal atau biner & Cohen's kappa \\
\hline
Lebih dari dua konfigurasi model penilai atau terdapat label hilang & Krippendorff's alpha dengan tingkat pengukuran yang sesuai \\
\hline
\end{tabularx}
}
\end{table}

Pedoman anotasi dan rubrik pada lampiran ini dibekukan bersama konfigurasi model serta artefak penilaian agar evaluasi otomatis dapat diulang. Pengujian pengguna nyata pada Subbab~\ref{sec:evaluasi-pengguna} memakai kuesioner untuk menangkap persepsi yang tidak dapat disimpulkan dari penilaian model.
...
\begin{figure}[!htbp]
  ...
  \caption[Alur penilaian buta RM1a]{Alur penilaian buta RM1a dari pembangkitan respons hingga agregasi preferensi dan \textit{weighted kappa}.}
...
\subsection{Protokol Kesesuaian Skor Total}

Protokol ini hanya digunakan bila subjek yang sama mengisi bentuk mandiri dan bentuk percakapan.

\begin{table}[!htbp]
\centering
\caption{Protokol pengumpulan pasangan skor}
{\footnotesize
...
\hline
Periode rujukan & Kedua bentuk menanyakan dua minggu yang sama \\
\hline
Urutan administrasi & Urutan bentuk mandiri dan percakapan dikontrabalanskan \\
\hline
Interval & Dicatat dan dibuat sesingkat yang layak tanpa mencampur sesi \\
\hline
Data yang dicatat & Skor per item, skor total, kategori, urutan, dan waktu administrasi \\
\hline
Analisis & ICC dan 95\% CI, Bland--Altman, selisih rerata, \textit{mean absolute error}, dan kesesuaian kategori \\
\hline
\end{tabularx}
}
\end{table}
...
\section{Korpus Emas Memori dan Lifecycle}
\label{lamp:v2-memory-gold}
\addcontentsline{loa}{lampiransub}{\protect\numberline{\thesection}Korpus Emas Memori dan Lifecycle}
...
\promptlisting[firstline=51,lastline=82]{../../evaluation_pipeline/rm1_rejudge.py}
...
Lampiran ini memuat transkrip apa adanya keenam skenario penilaian buta RM1a (Subbab~\ref{sec:v2-dialogue}): pesan pengguna, respons AXIS, respons \textit{baseline}, serta skor dan preferensi dari kedua konfigurasi model penilai. Label A/B pada tabel skor sesuai urutan penyajian ke penilai (diacak per skenario, berbeda dari label AXIS/\textit{baseline}). Konten disisipkan langsung dari artefak evaluasi tersimpan, bukan disalin manual, sehingga tidak dapat menyimpang dari data asli hasil eksekusi.
...
\hline
Kesepakatan antarkonfigurasi model penilai & Ya, untuk RM1a (Tabel~\ref{tab:v2-dialogue-result}); tidak berlaku untuk blok yang hanya memakai satu konfigurasi model \\
\hline
