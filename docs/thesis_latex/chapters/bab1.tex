...
Berangkat dari kesenjangan tersebut, tugas akhir ini mengembangkan AXIS, sebuah chatbot pendamping non-klinis untuk mahasiswa Indonesia. Fokusnya bukan melatih ulang model bahasa agar terdengar lebih ramah, melainkan menyusun sistem yang membuat tiga kemampuan berjalan bersamaan: percakapan reflektif yang tetap dalam batas non-klinis, mood harian dan asesmen skrining depresi PHQ-9 yang menyatu dengan alur cerita, serta memori jangka panjang yang dapat ditulis, dihubungkan, diperbarui, dan dipilih kembali ketika relevan. Kendali pengguna menyertai ketiga kemampuan tersebut agar personalisasi tidak mengambil alih keputusan pengguna.

Karena tugas akhir ini berada pada tahap pengembangan purwarupa, evaluasi dibatasi pada validitas rekayasa sistem. Bukti primer diperoleh melalui pengujian kontrak fungsional, \textit{benchmark} berlabel, penilaian buta terhadap respons, dan studi ablasi yang terkendali. Pengujian pengguna nyata dipakai untuk membaca pengalaman awal penggunaan purwarupa, bukan untuk membuktikan efektivitas klinis. Validasi dampak jangka panjang memerlukan studi longitudinal dengan protokol etik formal yang berada di luar cakupan tugas akhir ini\ifdefined\seminarhasilmode.\else. Keterbatasan ini dinyatakan eksplisit pada Keterbatasan BAB V (Subbab~\ref{sec:keterbatasan}) dan dijabarkan sebagai \textit{roadmap} pada Saran BAB V (Subbab~\ref{sec:saran}).\fi
...
\begin{enumerate}[label=\arabic*.,leftmargin=*,itemsep=4pt]
  \item \textbf{Percakapan pendamping.} Kualitas respons suportif dinilai melalui penilaian buta berbasis \textit{LLM-as-judge} terhadap respons AXIS dan \textit{baseline} pada skenario yang sama. Rubrik ordinal mencakup validasi emosi, pemahaman konteks, eksplorasi, ketepatan teknik CBT, kepatuhan batas non-klinis, kesesuaian ragam bahasa, dan \textit{groundedness} ketika memori digunakan. Pelaporan mencakup median atau distribusi skor per dimensi, preferensi berpasangan, dan kesepakatan antarkonfigurasi model penilai. Keselamatan dinilai melalui \textit{benchmark} berlabel dengan sensitivitas risiko tinggi sebagai metrik primer, disertai spesifisitas, presisi, $F_2$, jumlah \textit{false negative}, dan kepatuhan respons aman. Seluruh hasil dilaporkan pula menurut ragam bahasa formal, informal, bahasa campur, dan eufemistik.

  \item \textbf{Mood harian dan asesmen skrining depresi.} Fidelity orkestrasi dinilai melalui tingkat kelulusan kontrak untuk penawaran sukarela, penyampaian satu item per giliran, penghormatan penolakan, eskalasi item kesembilan, pemisahan data administratif dari memori, dan penyampaian hasil sebagai skrining. Pemetaan jawaban bebas ke skor 0 sampai 3 dinilai terutama dengan \textit{quadratic weighted kappa}, disertai \textit{macro-F1} dan \textit{accuracy}. Item kesembilan dinilai terpisah melalui sensitivitas, spesifisitas, dan ketepatan rute keselamatan. Apabila bentuk PHQ-9 biasa dan bentuk percakapan diisi oleh subjek yang sama, kesesuaian skor total dilaporkan dengan ICC dan analisis Bland--Altman.

  \item \textbf{Memori jangka panjang.} Evaluasi dipisahkan menjadi kemampuan menulis, mengambil, memperbarui, dan menggunakan memori. Penulisan dinilai dengan presisi, \textit{recall}, dan \textit{macro-F1} node serta relasi. Pengambilan dinilai dengan \textit{P@5}, \textit{Recall@5}, MRR, dan nDCG@5. Pembaruan dinilai melalui \textit{update correctness} dan \textit{stale-memory rate}. Penggunaan pada jawaban dinilai melalui \textit{grounded-answer rate}, tingkat kontradiksi, keberhasilan kueri relasional atau kausal, serta \textit{false-personalization rate}. Nilai tambah rancangan hibrid diuji melalui ablasi berpasangan terhadap kondisi vektor saja dengan interval kepercayaan 95\%.

  \item \textbf{Kendali pengguna.} Kendali pengguna dipertahankan sebagai kebutuhan pendukung lintas rumusan masalah, bukan sebagai klaim penelitian yang berdiri sendiri. Verifikasinya dilakukan melalui pengujian fungsional terhadap pengelolaan memori dan data personal serta melalui pengujian pengguna terhadap keberhasilan tugas dan pemahaman kontrol. Batas persistensi \textit{Confession Space} diverifikasi sebagai bagian dari kontrak fungsional.
\end{enumerate}
...
\begin{enumerate}[label=\arabic*.,leftmargin=*,itemsep=4pt]
  \item Sasaran utama sistem adalah mahasiswa Indonesia dewasa pada konteks non-klinis. AXIS tidak ditujukan untuk diagnosis, penentuan terapi, ataupun pengganti psikolog, konselor, psikiater, dan tenaga profesional. Asesmen depresi yang digunakan terbatas pada skrining PHQ-9.

  \item Domain percakapan dikalibrasi untuk tekanan akademik, relasi kampus, keluarga, organisasi, identitas diri, dan transisi karier mahasiswa Indonesia. Bahasa yang dicakup adalah bahasa Indonesia formal, informal, dan bahasa campur dengan istilah Inggris yang lazim di lingkungan kampus.

  \item Aplikasi web dan tampilan mobile berfungsi sebagai sarana penyajian serta pengujian pengalaman. Ruang percakapan suara sementara (\textit{Confession Space}) tidak ditulis ke memori jangka panjang, meskipun batas keselamatan tetap diterapkan.

  \item Evaluasi membuktikan validitas rekayasa purwarupa melalui pengujian kontrak, \textit{benchmark} berlabel, penilaian buta berbasis \textit{LLM-as-judge}, ablasi terkendali, dan pengalaman awal pengguna. Penelitian ini tidak menilai efektivitas klinis, penurunan gejala depresi, peningkatan kesejahteraan jangka panjang, atau keselamatan klinis penuh.
...
Bab IV, yaitu Implementasi dan Evaluasi, berfokus pada realisasi teknis dari rancangan pada Bab III dan evaluasi terhadap klaim sistem. Bab ini menyajikan implementasi arsitektur inti, pengujian kontrak fungsional, \textit{benchmark} berlabel untuk keselamatan, PHQ-9, dan memori, penilaian buta respons, studi ablasi retrieval, pengendalian keterulangan eksperimen, serta pengujian pengguna nyata terbatas.
