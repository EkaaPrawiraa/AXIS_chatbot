...
\begin{enumerate}[label=\arabic*.,leftmargin=*,itemsep=4pt]
  \item \textbf{Percakapan pendamping.} Kualitas respons suportif dinilai melalui penilaian buta berbasis \textit{LLM-as-judge} terhadap respons AXIS dan \textit{baseline} pada skenario yang sama. Rubrik ordinal mencakup validasi emosi, pemahaman konteks, eksplorasi, ketepatan teknik CBT, kepatuhan batas non-klinis, kesesuaian ragam bahasa, dan \textit{groundedness} ketika memori digunakan. Pelaporan mencakup median atau distribusi skor per dimensi, preferensi berpasangan, serta konfigurasi model dan rubrik penilai. Keselamatan dinilai melalui \textit{benchmark} berlabel dengan sensitivitas risiko tinggi sebagai metrik primer, disertai spesifisitas, presisi, $F_2$, jumlah \textit{false negative}, dan kepatuhan respons aman. Seluruh hasil dilaporkan pula menurut ragam bahasa formal, informal, bahasa campur, dan eufemistik.
