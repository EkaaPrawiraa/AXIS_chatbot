...
\begin{enumerate}[label=\arabic*.,leftmargin=*,itemsep=3pt]
  ...
  \item Pada pengulangan evaluasi tahap akhir, satu konfigurasi \textit{gemini-3.1-flash-lite} digunakan dengan suhu nol. Karena hanya ada satu penilai, ukuran kesepakatan antarpelabel tidak dilaporkan; batas ini dibedakan dari ketepatan sistem terhadap label yang dihasilkan penilai.
...
\begin{enumerate}[label=\arabic*.,leftmargin=*,itemsep=3pt]
  ...
  \item Satu konfigurasi model penilai buta, \texttt{gemini-3.1-flash-lite}, memberi skor ordinal 0 sampai 2 untuk reaksi emosional, interpretasi konteks, eksplorasi, ketepatan CBT, batas non-klinis, kesesuaian ragam bahasa, dan \textit{groundedness} apabila memori tersedia. Tiga dimensi pertama mengadaptasi EPITOME \parencite{sharma2020epitome}.
  \item Model penilai memilih preferensi A, B, atau setara untuk kualitas keseluruhan disertai alasan singkat.
  \item Laporkan median skor tiap dimensi dan proporsi preferensi. BLEU, ROUGE, dan metrik tumpang tindih teks tidak digunakan sebagai metrik utama.
\end{enumerate}
...
\hline
\textit{Groundedness} terhadap memori & 2,0 & 1,0 & 1,0 \\
\hline
\end{tabularx}
}
\end{table}
...
\hline
Seluruh skenario (18 skenario x satu penilai) & 10 & 8 & 0 \\
\hline
Tanpa memori (AXIS vs B0) & 4 & 5 & 0 \\
\hline
Dengan memori (AXIS vs B1) & 6 & 3 & 0 \\
\hline
\end{tabularx}
}
\end{table}
...
Pada 18 skenario satu-giliran, AXIS dipilih pada 10 penilaian dan \textit{baseline} pada delapan penilaian. Hasil ini tidak menunjukkan keunggulan percakapan umum: pada sembilan skenario tanpa memori, \textit{baseline} sedikit lebih sering dipilih (lima berbanding empat). Perbedaan yang paling jelas muncul pada sembilan skenario dengan memori. Median \textit{groundedness} AXIS adalah 2,0, sedangkan \textit{baseline} 1,0; AXIS dipilih pada enam dari sembilan kondisi tersebut. Enam dimensi lain memiliki median yang sama, sehingga bukti ini terutama mendukung penggunaan memori personal yang tepat, bukan superioritas empati secara umum.

Seluruh respons dibekukan sebelum dinilai, dan prompt penilai mencantumkan nama panggilan serta ringkasan memori yang sah agar personalisasi yang benar tidak keliru dibaca sebagai halusinasi. Karena evaluasi memakai satu penilai LLM Lite, hasil ini merupakan pembacaan berbasis rubrik dan belum mengukur kesepakatan antarpenilai maupun preferensi mahasiswa.
...
\hline
Formal & \multicolumn{2}{c|}{Belum diuji pada rubrik buta} & 0,500 (n=4) & Tidak berlaku \\
\hline
Informal & 2,0 & 10/18 & 0,778 (n=16) & 0/18 skenario \\
\hline
Bahasa campur & \multicolumn{2}{c|}{Belum diuji pada rubrik buta} & 0,500 (n=21) & Tidak berlaku \\
\hline
Eufemistik & Belum diuji pada rubrik buta & Belum diuji pada rubrik buta & 0,667 (n=9) & Tidak berlaku \\
\hline
\end{tabularx}
}
\end{table}

Skor bahasa dan preferensi pada baris Informal berasal dari 18 skenario RM1a (Subbab~\ref{sec:v2-dialogue}) yang menggunakan bahasa Indonesia informal khas mahasiswa. Sensitivitas risiko per ragam berasal dari korpus 50 pesan pada Subbab~\ref{sec:v2-safety}. Nilai formal, bahasa campur, dan eufemistik masing-masing didukung empat, 21, dan sembilan pesan; angka tersebut berguna untuk menunjukkan cakupan, tetapi belum cukup untuk membandingkan ragam secara statistik. Belum ada penilaian buta respons suportif yang terpisah untuk tiga ragam tersebut.
...
Pada 80 jawaban bebas yang mencakup seluruh item, 73 jawaban dengan frekuensi eksplisit dipetakan sama dengan label acuan penilai ($QWK=1{,}00$ dan \textit{macro-F1}=1{,}00). Tujuh jawaban tanpa frekuensi yang cukup jelas seluruhnya memicu klarifikasi. Hasil ini menunjukkan pemetaan bekerja pada korpus terarah yang diuji; hasil tersebut menilai fidelity pemetaan bahasa ke pilihan PHQ-9, bukan validitas psikometrik atau diagnosis.
...
\begin{enumerate}[label=\arabic*.,leftmargin=*,itemsep=3pt]
  ...
  \item \textbf{Mengambil memori.} Metrik \textit{P@5}, \textit{Recall@5}, MRR, dan nDCG@5 digunakan untuk mengukur peringkat kandidat relevan \parencite{jarvelin2002cumulated}. Probe internal menggunakan fakta acuan, sedangkan LongMemEval_S menggunakan label sesi bukti resmi.
  ...
  \item \textbf{Menggunakan memori.} Konfigurasi penilai LLM yang direkam pada artefak memeriksa apakah jawaban didukung memori aktif, bertentangan dengan memori, berhasil menjawab kueri relasional atau kausal, dan melakukan personalisasi tanpa memori relevan. Metrik yang dituju adalah \textit{grounded-answer rate}, \textit{contradiction rate}, tingkat keberhasilan kueri relasional atau kausal, dan \textit{false-personalization rate}; metrik ini belum dihitung karena pasangan jawaban berlabel belum tersedia.
\end{enumerate}
...
\hline
Kalibrasi retrieval eksternal & LongMemEval_S, 12 kueri; P@5 / \textit{Recall@5} / MRR / nDCG@5 & 0,367 / 0,972 / 0,833 / 0,855 \\
\hline
...
\hline
\textit{P@5} & \multicolumn{3}{c}{Tidak dibandingkan pada blok ablasi; dilaporkan terpisah pada kalibrasi LongMemEval_S} \\
\hline
\textit{Recall@5} (kueri parafrase satu \textit{hop}) & 1,00 (15/15) & 1,00 (15/15) & 0,00 (95\% CI \textit{bootstrap} [0,00; 0,00]) \\
\hline
MRR & \multicolumn{3}{c}{Tidak dibandingkan pada blok ablasi; dilaporkan terpisah pada kalibrasi LongMemEval_S} \\
\hline
nDCG@5 & \multicolumn{3}{c}{Tidak dibandingkan pada blok ablasi; dilaporkan terpisah pada kalibrasi LongMemEval_S} \\
\hline
...
P@5, MRR, dan nDCG@5 tersedia pada kalibrasi LongMemEval_S di Tabel~\ref{tab:v2-memory-result}, tetapi tidak dibandingkan antar-kondisi karena korpus tersebut belum dianotasi sebagai graf AXIS. Metrik pembaruan dan penggunaan memori pada jawaban (\textit{update correctness} hingga \textit{false-personalization rate}) tetap memerlukan korpus kasus dan pasangan jawaban berlabel. Klaim keunggulan kondisi hibrid pada tahap ini dibatasi pada kemampuan menjawab kueri yang mengandalkan identitas relasi graf, bukan pada seluruh kemampuan memori jangka panjang secara umum. Bukti kontrak lifecycle dan probe reappraisal dilaporkan terpisah karena keduanya menguji transisi memori, bukan membandingkan kedua kondisi retrieval.
