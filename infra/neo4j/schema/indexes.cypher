...
// [LOOKUP] Sentiment filter -- Subject nodes with all-negative sentiment
// after 3+ sessions trigger social reconnection prompt (Haque & Rubya, 2023).
CREATE INDEX subject_sentiment_idx IF NOT EXISTS
  FOR (su:Subject)
  ON (su.sentiment);

// [LOOKUP] Mention count -- most mentioned people surface first in
// psychologist pre-screening profile.
CREATE INDEX subject_mention_count_idx IF NOT EXISTS
  FOR (su:Subject)
  ON (su.mention_count);

// [LOOKUP] Role filter -- e.g., "family", "friend", "professor".
CREATE INDEX subject_role_idx IF NOT EXISTS
  FOR (su:Subject)
  ON (su.role);
...
CREATE FULLTEXT INDEX subject_name_ft_idx IF NOT EXISTS
  FOR (su:Subject)
  ON EACH [su.name];
