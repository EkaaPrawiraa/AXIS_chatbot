    ...
    <div className={`fixed inset-0 z-[85] bg-black/40 ${animationClasses.sheetBackdropIn}`} onClick={onClose}>
      ...
      >
        ...
        <div className="mt-4 space-y-3">
          <div>
            <label className="text-[12.5px] font-bold text-[#6b7355]">
              Ketik <span className="text-[#c05b33]">{CONFIRM_WORD}</span> untuk konfirmasi
            </label>
            <input
              value={confirmText}
              onChange={(event) => setConfirmText(event.target.value)}
              placeholder={CONFIRM_WORD}
              className="mt-1 w-full rounded-[12px] border border-[#e5d6c2] bg-white/70 px-3.5 py-2.5 text-[14.5px] font-bold text-[var(--v2-ink)] outline-none"
            />
          </div>
          <div>
            <label className="text-[12.5px] font-bold text-[#6b7355]">Masukkan password kamu</label>
            <input
              type="password"
              value={password}
              onChange={(event) => setPassword(event.target.value)}
              placeholder="Password"
              className="mt-1 w-full rounded-[12px] border border-[#e5d6c2] bg-white/70 px-3.5 py-2.5 text-[14.5px] font-semibold text-[var(--v2-ink)] outline-none"
            />
          </div>
        </div>

        {errorMessage ? (
          <p className="mt-3 rounded-[12px] bg-[#f6e3d3] px-3.5 py-2 text-[13px] font-semibold text-[#a34a28]">
            {errorMessage}
          </p>
        ) : null}
        ...
        <div className="mt-5 flex items-center gap-3">
          ...
          <button
            ...
            disabled={!canSubmit}
            className="v2-anim-pressable flex h-[52px] flex-1 items-center justify-center gap-2.5 rounded-full bg-[var(--v2-clay)] text-[15.5px] font-bold text-white shadow-[0_14px_26px_-14px_rgba(195,108,69,0.9)] disabled:opacity-40"
          >
